...
class AulasAcessadas {
    ...
    static rotas(app) {                                  
        ...
        app.get("/aulasAcessadas/:id", async(req, res) => {
            try {
                const aulas = await databaseAulasAcessadas.listarAulasAcessadasPorId(req.params.id);
                if(!aulas) {
        ...
        app.post("/aulasAcessadas", async(req, res)=> {
            try {
                ...
                const resposta = await databaseAulasAcessadas.inserirAulaAcessada(aulas);
        ...
        app.put("/aulasAcessadas/:id", async(req,res)=> {
            try {
                const aulaId = await databaseAulasAcessadas.listarAulasAcessadasPorId(req.params.id);
                if(!aulaId) {
                    throw new Error("Trilha não encontrada para esse Id.")
                }
                const aulas = new aulasAcessadasModel(...Object.values(req.body));
                const resposta = await databaseAulasAcessadas.atualizarAulaAcessadaPorId(req.params.id, aulas);
                res.status(200).json(resposta);
            } catch (error) {
                res.status(400).json(error.message);
              }
           ...
           app.delete("/aulasAcessadas", async(req, res)=> {
            try {
                const aulas = new aulasAcessadasModel(...Object.values(req.body));
                const resposta = await databaseAulasAcessadas.deletarAulasAcessadas(aulas);
                res.status(201).json(resposta);
            } catch(error) {
                res.status(400).json(error.message)
            }
        ...
        app.delete("/aulasAcessadas/:id", async(req,res)=> {
            try {
                ...
                const aula = await databaseAulasAcessadas.deletarAulaAcessadaPorId(req.params.id);
